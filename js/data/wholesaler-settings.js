...
export async function getWholesalerSettings(wid) {
  const { data } = await sbCall(
    supabase.from("v2_wholesalers").select("wid,low_moq_threshold,trust_message,return_policy,payment_terms").eq("wid", wid).maybeSingle()
  );
  return data || { wid, low_moq_threshold: 12, trust_message: null, return_policy: null, payment_terms: null };
}
