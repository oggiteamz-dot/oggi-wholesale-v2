...
**What the revamp did NOT cover, and is therefore still open:** rows 4, 36, 37,
38 and 39 of `FEATURE-MANIFEST.md` — reorder minimums, kit assembly, landed
...
2. **Landed cost** (row 37) — feeds the valuation numbers a wholesaler might
   take to a bank, and is still two unreconciled numbers with `variants.cost`.
3. **CSV import/export** (row 51) — already caused one silent loss, when the
   importer hardcoded `sellMode: "open"` over every wholesaler's selling model.
4. **Kit assembly, cycle counts, suppliers, AI import, integrations** (36, 38,
   39, 52, 53).
