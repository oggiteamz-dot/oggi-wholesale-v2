...
| `js/views/buyer.js` | 4 | `skeletonWrap.remove()` → an idempotent `clearSkeleton()` called where the shop actually goes up, and three `outlet.appendChild(emptyState(...))` → `bail(emptyState(...))`, which is `clearSkeleton()` then that same append. Every empty state renders exactly as before. |
