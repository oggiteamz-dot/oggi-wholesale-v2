...
| 457 | **The marketplace now READS the product flag.** 116 put `is_public` on the product and nothing read it — a column no code touches is the dead-machinery pattern this repo already carries (`v2_size_ratios` exports 9 functions with no caller). 119 changes six lines: `v2_marketplace_feed` and `v2_marketplace_search` stop reaching publicness *through* `join v2_catalog_products → join v2_catalogs → where c.is_public` and ask `p.is_public` directly. Zero catalogue joins remain in either body. Nothing a buyer sees moved: seven result hashes taken before and after are byte-identical, and both Atelier leak probes stayed at 0 rows | `migrations/119`, `v2_marketplace_feed`, `v2_marketplace_search` | `check_marketplace_reads_product_flag.sql` — 12 assertions, BEHAVIOURAL (a flagged uncatalogued product must appear; an unflagged product in a public catalogue must not), red-proved 2 ways | ✅ |
| 458 | **The feed and the search may not disagree about what "public" means.** They are two functions carrying one rule, so 119 changes both in a single migration. When they drift the WIDER one wins silently: a private line is simply in the results and nothing on any screen looks broken. The gate builds one fixture, reaches it through both functions, and asserts the two sets are equal | `migrations/119` | `check_marketplace_reads_product_flag.sql` assertion 8 — red-proved by reverting **only** the search, which fired 6, 7, 8 and 11 | ✅ |
| 459 | **A migration may assert things about the CHANGE it makes; it may not assert things about the DATA it finds.** 119 shipped with a guard refusing to apply when no product was public. On an empty database no product is public, so the migration refused and `replay_migrations.sh` stopped at file 121 of 121 — the repo could no longer rebuild production. This is the **second** time in two days: 116 was corrected for the identical mistake the day before, which is why the rule is now written as a rule rather than as a story about one migration. The *gained/lost* comparison stays in 119 (true on every database); the corpus property moved into the gate, which brings its own fixture | `migrations/119`, `migrations/116` | `checks/replay_migrations.sh` — 121 migrations into an empty Postgres 16, and `check_marketplace_reads_product_flag` then run against THAT database, 12/12 | ✅ |
| 460 | **The tier gate is removed — a member of a store sees that store.** D2, in Hadi's words: *"Tier two, gate. Drop it. Completely remove it."* `access_tier` was compared in exactly two places (`v2_buyer_catalogs` and `v2_catalog_by_token`) and both comparisons go in one migration, because a list and a link that disagree about who may open a catalogue is the MOD-03 drift again. Measured on production first: 8 of 26 catalogues sat above tier 1, so 7 of 13 buyer accounts can now enumerate one more catalogue — **and not one product changes hands**, all 13 product-set hashes byte-identical, because MOD-04 already widened the store read to the union. Atelier's three made-to-order gowns sit behind tier 5 and that store's only buyer is already tier 5, so they do not move; they stay out of the marketplace, which is `p.is_public` and a different rule | `migrations/120`, `v2_buyer_catalogs`, `v2_catalog_by_token`, `js/views/wholesaler.js` | `check_tier_gate_removed.sql` — 15 assertions, red-proved 2 ways | ✅ |
| 461 | **⭐ The tenant boundary survived the tier gate's removal.** `v2_catalog_by_token` returned `'denied'` from **two adjacent blocks** — `v_acct.wid is distinct from v_cat.wid` (which SHOP you belong to) and `v_tier < v_cat.access_tier` (your RANK inside it) — same string, four lines apart. Deleting the wrong one opens every wholesaler's private catalogues to every other wholesaler's customers while the screen still says "denied" often enough to look fine. Every "the gate is gone" assertion is paired with a "and this is still shut" assertion, in both directions | `migrations/120` | `check_tier_gate_removed.sql` assertions 3, 4, 7, 9, 10 — red-proved by deleting the tenant check **instead of** the tier gate, which fired all five while assertions 1, 2, 6 and 8 still passed | ✅ |
| 462 | **Seven assertions across five gates were turned around, not deleted.** `check_buyer_catalog_access`, `check_catalog_link_access`, `check_catalog_read`, `check_buyer_pricing` and `check_catalog_packs` all correctly encoded the tier rule and went red the moment MOD-07 landed. Each tier row was **inverted** — *"a tier 1 buyer is NOT shown a tier 3 catalog"* became *"IS now shown"* — so the same code path is still exercised and reinstating the gate still turns them red. Deleting them would have removed the coverage along with the obsolete expectation. Every negative row about a **different wholesaler**, an inactive catalogue, a dead token or a deactivated account was green throughout and untouched | `checks/check_buyer_catalog_access.sql`, `checks/check_catalog_link_access.sql`, `checks/check_catalog_read.sql`, `checks/check_buyer_pricing.sql`, `checks/check_catalog_packs.sql` | themselves — proven by a control replay built from `origin/main`, which isolated these 7 real failures from 18 pre-existing ones | ✅ |
| 463 | **The discount stack is ADDITIVE, and that is now proven on an invoice rather than on a percentage.** MOD-06 asked for *"a store at 10% + a customer at 5% invoices correctly."* `check_store_pricing_dial` already proved `v2_store_discount_pct` **returns 15**; nothing proved what a buyer is **charged**. Two discounts can stack two ways — additive `100×(1−0.15)=85.00` or compounding `100×0.90×0.95=85.50` — and the difference is fifty cents on a hundred, which is not a crash, not a wrong screen, just a slightly wrong bill. So the rule is pinned from both sides: 85.00 required **and** 85.50 forbidden by name. The gate also pins the order of operations (a quantity break is chosen first and the discount comes off the **broken** price: 12 × 80.00 less 15% = 816.00, not 960.00 and not 1020.00), the `customer_only` 0%-falls-back-to-the-shelf quirk **as money**, a negative dial only partly offset by a customer rate (−10 + 5 → 105.00), the 0.00 floor, and per-**unit** rounding (0.99 less 15% = 0.84 each, seven for 5.88 — rounding the line would give 5.89, one penny on every line forever) | `v2_effective_unit_price`, `v2_catalog_discount_pct`, `v2_submit_order` | `check_discount_stacking.sql` — 21 assertions, every one a number written onto `v2_order_items`; red-proved **4 ways** (compounding fired 6, dropping the override short-circuit fired 1, removing the `greatest(…,0)` floor fired 2, skipping the quantity break fired 1) | ✅ |
| 464 | **A negotiated price wins outright — the discount stack is not applied on top of it.** `v2_effective_unit_price` returns a `v2_client_price_overrides` row and stops; the shelf rate and the customer rate are both discarded. The gate names **both** wrong answers rather than only the right one — 71.00, not 85.00 (as if the override were ignored) and not 60.35 (as if 15% came off it as well) — because "it returned a plausible number" is exactly how this class of bug hides. Measured on production: **19 overrides across 5 clients**, and every one is already deeper than that customer's own percentage would have been, so override-wins costs no buyer money today | `v2_effective_unit_price`, `v2_client_price_overrides` | `check_discount_stacking.sql` assertions 9 and 9b — red-proved by deleting the short-circuit, which fired 9 and nothing else | ✅ |
| 465 | **⚠️ OPEN, AND IT IS MONEY — the same buyer is charged two prices for the same shirt depending on which door they used.** Since MOD-04 a buyer **sees** the whole store but is **priced** through one shelf (`activeCatalogId`, which is the default shelf, dial 0%), while `v2_token_discount_pct` prices a share link through the shelf the link names. Measured on production after MOD-07: **326 (account, variant) pairs disagree.** Aïsha Couture at demo-atelier, customer rate 15%, A-101 Silk Slip Dress listing at 128.00 — **108.80 through the store screen, 96.00 through the "Occasion Private Edit" link**, same buyer, same afternoon, 12.80 a unit apart. Nothing on either screen is wrong; the screen and the invoice agree with each other. What disagrees is the two doors, and closing it moves prices, so it is Hadi's call and not mine. Recorded, not fixed | `js/views/buyer.js:100`, `v2_effective_unit_price`, `v2_token_discount_pct` | `check_discount_stacking.sql` assertions 15 and 15b — asserted as **today's truth** with the instruction to *invert*, not delete, when the decision is made | ⚠️ |
| 466 | **⚠️ OPEN — the store pricing dial is bounded by nothing.** `v2_catalogs.discount_pct` carries `v2_catalogs_discount_range` (−100…100), and the gate proves a shelf refuses 500%. The dial migration 117 added to `v2_wholesalers` constrains only `discount_mode`; the **percentage is unconstrained**, so a store can be set to 500% or −5000%. It has cost nothing so far because nothing reads it — MOD-05 left it deliberately inert. The moment the switch-over happens it is the number that prices **every order in the store**, with a fat-fingered keypress and no check between it and the invoice. A one-line migration fixes it | `migrations/117`, `v2_wholesalers.discount_pct` | `check_discount_stacking.sql` assertions 16 and 17 — 16 proves the shelf **is** bounded, 17 records that the store is **not**, with the instruction to invert when it is | ⚠️ |
| 467 | **The marketplace switch is reachable at last — MOD-01 built the flag, MOD-03 read it, and nothing in the app could write it.** Between those two migrations landing and this row, `v2_products.is_public` was the sole thing deciding what the OGGI marketplace showed, and **no screen, RPC or data module could set it**. A wholesaler could not publish a new product and could not take an old one down; the marketplace was frozen at whatever migration 116's backfill happened to produce, and nothing anywhere said so. `setProductPublic()` is the writer, the Products screen carries the control, and a published product is badged so the toggle's position is visible rather than something you learn by pressing it. The button names the RESULT — *"Put on the marketplace"* / *"Take off the marketplace"* — and its tooltip states the two things the screen cannot show: that this is separate from the catalog link, and that `w.active` and `p.archived` still filter at read time | `js/data/products-admin.js`, `js/views/wholesaler.js` | `check_marketplace_switch_reachable.mjs` — 10 assertions, red-proved **5 ways**, each sabotage caught by exactly one row | ✅ |
| 468 | **⭐ The gate's load-bearing assertion is "a view actually CALLS it", and it is there because of `createRatio()`.** An export nothing calls is this repo's most repeated defect and has never once been caught: `createRatio()` is the only function that creates a size ratio, has had no caller since 24 August, and every wholesaler onboarded since has an empty ratio list forever — 9 of that module's 12 exports are unreachable and nothing was ever red, because everything that exists works. So the gate checks reachability as a chain — written by a data module, exported, imported by a view, **called** from that view, and its state shown on screen — rather than checking that the toggle is correct. Deleting the call site while leaving the import in place fires assertion 4 and nothing else | `checks/check_marketplace_switch_reachable.mjs` | itself — red-proved by removing the call site and keeping the import, which is precisely the `createRatio()` shape | ✅ |
| 469 | **The public checkbox stops lying, in both directions — and the correction is written on the screen, not only in the code.** It read *"Open to anyone with the link. No login."* and also published every product on that shelf to the marketplace. MOD-03 ended that. But every wholesaler who used the box before today learned the old behaviour, and a silently narrowed control is its own kind of lie — so the label now says, in words, that it does not publish to the marketplace and where the switch that does now lives. `setCatalogPublic` is asserted to touch `v2_catalogs` and **not** `v2_products`, so the two switches cannot merge back into one. A stale comment in `js/data/marketplace-feed.js` still named the catalogue's flag as the feed's scope — correct until migration 119, wrong the moment it landed — and assertion 9 forbids that sentence returning; **it fired on the corrected comment's first run, because the correction quoted the old wording in order to explain it** | `js/views/wholesaler.js`, `js/data/catalogs.js`, `js/data/marketplace-feed.js`, `css/components.css` | `check_marketplace_switch_reachable.mjs` assertions 7, 8 and 9 | ✅ |
| 470 | **⚠️→⭐ A control run that covers only some of the gates measures its own blind spot.** MOD-07's evidence records a control replay that isolated *exactly seven* failing assertions across five gates, and it was wrong: the control ran only the **SQL** gates. There were **eight**, and the eighth was in a `.mjs` one — `check_catalog_builder.mjs` asserted `cat-tier … cat-discount … cat-mode` were all present in that order, and MOD-07 removed the tier control. **That gate was red on `main` for about an hour and I did not notice**, which is the same shape as row 451, where the manifest gate sat red on `main` for five days. The row is inverted rather than deleted, the way the other seven were: it now asserts the two remaining settings in order **and** that the tier control is gone | `checks/check_catalog_builder.mjs` | itself — 45 assertions green; found by running **every** `.mjs` gate against a control tree built from `main`, which is the run that should have happened the first time | ✅ |
| 471 | **⭐ CNT-00 — the whole-product receive screen. 250 can no longer become 270.** `receive-dialog.js` is 192 lines and takes ONE variant and ONE number, so receiving a style in 4 colours and 4 sizes means opening it **sixteen times** — and nothing in the system adds them up, and nothing compares the total to the vendor's invoice. There was no screen on which a wholesaler could see what he had entered against what he was billed: the error was not merely possible, there was no mechanism by which it could be caught. The new screen types the billed figure **once**, derives the total from the grid, and refuses to save while they disagree. One card per colour with sizes wrapping, so nothing scrolls sideways (CNT-07); whole numbers with no spinners (CNT-05); row and column totals always on screen (CNT-06). `receive-dialog.js` is untouched and still the right tool for correcting one box | `js/components/receive-product.js`, `js/lib/variant-grid.js`, `js/views/wholesaler.js` | `check_receive_count.mjs` — 42 assertions driving the real component in a real DOM, red-proved **9 ways** | ✅ |
| 472 | **⭐ The count check is a DATABASE rule, not a disabled button.** CNT-03 says blocked, not warned. A disabled button is a UI state: it survives exactly as long as this screen is the only way in, and this codebase already has a CSV import path and a barcode path that write stock without going near it. So migration 121's `v2_receive_product` adds the breakdown up **itself** and refuses if it does not equal the billed figure, in one transaction — sixteen round trips from a browser is not atomic, and closing the laptop at box nine leaves a warehouse holding nine boxes of a delivery nothing records as half-done. **There is no override argument, deliberately**: CNT-10 is a separate decision, and a parameter added "for later" is one somebody uses today. The refusal names both numbers and the direction — *"the breakdown comes to 270 pieces and the invoice says 250 — 20 too many"* | `migrations/121`, `js/data/inventory-admin.js` | `check_receive_count.sql` — 19 assertions, red-proved **6 ways**; assertion 3b is the one that matters, proving a refused receipt leaves the warehouse holding **exactly** what it held before | ✅ |
| 473 | **The fill tools, because the gate is necessary and not sufficient.** Nobody types 32 numbers correctly. Spread evenly (CNT-11) places every piece and **names where the remainder went** (CNT-09) — *"250 does not divide by 15 — 16 in each box, and the extra 10 went to Navy S, Navy M…"*. A size curve (CNT-12) shares the invoice by weights and shows the live arithmetic the spec asked for and no screen had: one set, one full run across the colours, how many runs fit, what is left over. Fill a colour or a size (CNT-13). Paste from a packing list (CNT-14), because vendors send Excel and retyping it is the largest single source of error we have — matched by size **name** where the paste carries a header, by position only where it does not. One-level undo on every bulk action (CNT-08) | `js/components/receive-product.js` | `check_receive_count.mjs` assertions 8–14 and 16 | ✅ |
| 474 | **CNT-15 — `createRatio()` has a caller at last, 13 days after it was written.** It is the only function in the codebase that CREATES a size curve, it has had no caller since 24 August, and every wholesaler onboarded since has an empty curve list forever; 9 of that module's 12 exports are unreachable. "Save this curve" on the receive screen is its first one, and "Fill now" beside each saved curve is the spec's *"next delivery of that style is one tap"* — the whole return on the feature, because nobody types sixteen numbers correctly **twice**. NOTE: the spec said CNT-12 was *"`applyRatio()` wired up at last"*; **the code disagrees** — `applyRatio()` calls `v2_apply_ratio` and builds packs, it does not spread a quantity across a grid. What the fill needs is the weights, which `listRatios()` already returned | `js/data/size-ratios.js` (unchanged), `js/components/receive-product.js` | `check_receive_count.mjs` assertions 10, 11 and 11b | ✅ |
| 475 | **Four grid helpers moved out of `order-setup.js` into `js/lib/variant-grid.js` (ALLOW_DELETIONS=1, 55 lines).** `coloursOf`, `sizesOf`, `variantAt` and `colourMeta` were private to the pack builder; the receive screen needs the same four answers, and this repo's own rule is that a helper copy-pasted into two places is a bug waiting for one copy to be fixed. If the two screens ever disagreed about which colours a product has, a wholesaler would be looking at a grid on one screen that does not exist on the other and there would be no way to tell which was right. **A move, not a removal**: `check_order_setup.mjs` passes all 27 of its assertions afterwards, which is what says the behaviour is unchanged | `js/lib/variant-grid.js`, `js/components/order-setup.js` | `check_order_setup.mjs` (27/27 after the move), `check_cross_module_imports.mjs`, `check_no_undeclared_identifiers.mjs` | ✅ |
| 476 | **⭐ The gate was blind to the one bug that would have been invisible, and a sabotage found it.** Nine sabotages were run against the receive screen; eight went red and one — *"the curve fills by POSITION instead of by NAME"* — passed, because every assertion applied a curve whose sizes were the screen's own, where the two rules agree. A saved curve for **M-L** applied to a product with **S-M-L-XL** would have put the 4 on S and the 6 on M: the total right, the boxes wrong, and nothing on screen to notice. The fix was to make the by-name path reachable and useful ("Fill now" straight from a saved curve) and then assert it. **A sabotage that does not go red is a finding about the gate, not a clean bill of health** | `checks/check_receive_count.mjs` assertion 11b | itself — the same sabotage now fires 2 assertions, showing the 4 landing on S and the 6 on M | ✅ |
| 477 | **⚠️ OPEN, AND IT IS A HOLE — `v2_receive_stock` is `SECURITY DEFINER`, granted to `anon`, and checks nothing.** It takes a variant id and a location id from the caller and adds stock. Anyone holding the anon key that ships in the bundle can inflate any wholesaler's inventory. It is **not fixed here**: revoking a grant the live CSV-import and barcode paths may depend on is not a change to make unattended, and the fix needs a look at which callers run signed-in. What migration 121 does is refuse to **repeat** it — `v2_receive_product` is `authenticated` only, and every variant must belong to the warehouse's own store | `v2_receive_stock`, `migrations/001` | `check_receive_count.sql` assertion 15 — proves the NEW door is shut to anon; the old one is recorded in GATE-EVIDENCE for Hadi | ⚠️ |
| 478 | **⚠️ NOT BUILT, and deliberately not made to look built — CNT-02 and CNT-10.** CNT-02 stores `billed_qty` on the line, so the record of what the vendor charged for survives after the count balances. CNT-10 lets a short delivery through with a typed reason written to the audit log. Both need a receipt header row that does not exist — receiving writes one movement per variant with nothing to hang an invoice figure on — and CNT-10 is a decision only Hadi can make ("may a wholesaler override a short count at all, or never?"). Tonight's work stops 250 becoming 270; the record of the invoice, and the licence to disagree with it, is the next one | — | — | ⚠️ |
| 479 | **A separator nobody can see was load-bearing, and a source file had turned binary.** The receive grid keys its cells `colour + separator + size`. A space is wrong: a colour called *"Navy Blue"* with size *"S"* collides with a colour *"Navy"* and a size *"Blue S"*, and the two silently share one box. U+0000 is the right separator and cannot be typed into a collision — but it was written as a **literal NUL byte**, which made `grep` report *"binary file matches"* and would have made the file unreadable to git, editors and code review. It is now written as an escape and exported as `cellKey()`, so the gate builds its keys the way the module does instead of assuming the separator | `js/components/receive-product.js` | `check_receive_count.mjs` assertion 16 — which is how the NUL was found: the paste assertions failed while everything else passed | ✅ |
...
## Reconciliation — 6 September 2026 (MOD-01 the product public flag, MOD-05 the store pricing dial, MOD-04 the whole store, MOD-03 the marketplace reading that flag, MOD-07 the tier gate removed, MOD-06 the discount stack proven on the invoice, MOD-08 the public checkbox and the marketplace switch, CNT-00 the whole-product receive screen, and four gates found red on main) and 1 September 2026 (MK-01/02/03/04, the login doors, the size order) and 30 August 2026 (SR-07, SR-05, AC-08/09/17, AC-07/11 + PB-01) and 28–29 August 2026 (Batch S, Batch N 1–4, the Client View gaps, AC-01, Door A, ID-01)

| | |
|---|---|
| Features listed | **479** |
| Enforced and proven (✅) | **457** |
| Present but unproven (⚠️) | **22** |
| Not built (❌) | **0** |
